...
from controller.settings import UEditorUploadSettings, UEditorSettings
from controller.utils import FileSize

#这个controller模块是为将ueditor和django结合
...
def get_ueditor_settings(request):
    return HttpResponse(json.dumps(UEditorUploadSettings, ensure_ascii=False),
...
def list_files(request):
    ...
    allowFiles = {
        "listfile": UEditorUploadSettings.get("fileManagerAllowFiles", []),
        "listimage": UEditorUploadSettings.get("imageManagerAllowFiles", [])
    }
    listSize = {
        "listfile": UEditorUploadSettings.get("fileManagerListSize", ""),
        "listimage": UEditorUploadSettings.get("imageManagerListSize", "")
    }
    listpath = {
        "listfile": UEditorUploadSettings.get("fileManagerListPath", ""),
        "listimage": UEditorUploadSettings.get("imageManagerListPath", "")
    }
    # 取得参数
    ...
    files = []
    root_path = os.path.join(settings.MEDIA_ROOT, listpath[action]).replace("\\", "/")
...
def UploadFile(request):
    ...
    if action in upload_allow_type:
        allow_type = list(request.GET.get(upload_allow_type[action],
                                          UEditorUploadSettings.get(upload_allow_type[action], "")))
    ...
    max_size = long(
        request.GET.get(upload_max_size[action], UEditorUploadSettings.get(upload_max_size[action], 0)))
    if max_size != 0:
    ...
    # 返回数据
    return_info = {
        'url': urljoin(settings.MEDIA_URL, OutputPathFormat),  # 保存后的文件名称
...
def catcher_remote_image(request):
    ...
    allow_type = list(
        request.GET.get("catcherAllowFiles", UEditorUploadSettings.get("catcherAllowFiles", "")))
    max_size = long(request.GET.get("catcherMaxSize", UEditorUploadSettings.get("catcherMaxSize", 0)))
...
def get_output_path(request, path_format, path_format_var):
    # 取得输出文件的路径
    OutputPathFormat = (
            request.GET.get(path_format, UEditorSettings["defaultPathFormat"]) % path_format_var).replace(
        "\\", "/")
    # 分解OutputPathFormat
